define([
  ...
  function w(e, t, a) {
    ...
    ;(s[c++] = e), (s[c++] = r)
    for (let e = 0; e < r; ++e)
      i.OrientedBoundingBox.pack(t[e], s, c),
        (c += i.OrientedBoundingBox.packedLength)
    const f = a.length
    s[c++] = f
    for (let e = 0; e < f; ++e) {
      const t = a[e]
      n.Color.pack(t.color, s, c),
    ...
    for (D in $)
      $.hasOwnProperty(D) &&
        ...
        ae.push({
          ...
          offset: ee.indexOffset,
          count: ee.indexLength,
          batchIds: ee.batchIds
        }))
    for (M = 0; M < H; ++M) {
      ;(D = p[M]), (ee = $[D])
      const e = ee.offset
      ...
      ;(U = L),
        ...
        (U.south = b),
        (U.north = x),
        (y[M] = i.OrientedBoundingBox.fromRectangle(U, P, F, O))
