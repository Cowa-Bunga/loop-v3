  ...
  ;(d.unpack = function (t, i, n) {
    ...
    (d.createGeometry = function (t) {
      ...
      const L = new Array(x),
        R = new Array(x)
      for (O = 0; O < x; O++)
        (T = p + (O * (h - p)) / (x - 1)), (L[O] = c(T)), (R[O] = f(T))
      const N = new Array(y),
        B = new Array(y)
      for (O = 0; O < y; O++)
        (I = C + (O * (_ - C)) / (y - 1)), (N[O] = c(I)), (B[O] = f(I))
      for (O = 0; O < x; O++)
        for (D = 0; D < y; D++)
          (g[z++] = l.x * L[O] * B[D]),
            (g[z++] = l.y * L[O] * N[D]),
        ...
        for (O = 0; O < x; O++)
          for (D = 0; D < y; D++)
            (g[z++] = d.x * L[O] * B[D]),
              (g[z++] = d.y * L[O] * N[D]),
              (g[z++] = d.z * R[O])
      for (L.length = y, R.length = y, O = 0; O < y; O++)
        (T = p + (O * (h - p)) / (y - 1)), (L[O] = c(T)), (R[O] = f(T))
      for (N.length = b, B.length = b, O = 0; O < b; O++)
        (I = C + (O * (_ - C)) / (b - 1)), (N[O] = c(I)), (B[O] = f(I))
      for (O = 0; O < y; O++)
        for (D = 0; D < b; D++)
          (g[z++] = l.x * L[O] * B[D]),
            (g[z++] = l.y * L[O] * N[D]),
      ...
      for (O = 0; O < b; O++)
        for (D = 0; D < y - 1; D++)
          (E[z++] = S + O + D * b), (E[z++] = S + O + (D + 1) * b)
      if (v)
        for (S = x * y * P + b * y, O = 0; O < b; O++)
          for (D = 0; D < y - 1; D++)
            (E[z++] = S + O + D * b), (E[z++] = S + O + (D + 1) * b)
      if (v) {
        let t = x * y * P,
      ...
      if (a.defined(t._offsetAttribute)) {
        ...
        U.applyOffset = new o.GeometryAttribute({
