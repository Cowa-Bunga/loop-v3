define([
  ...
  const A = {
    ...
    toWebGLConstant: function (_, t) {
      switch (_) {
        ...
        case A.HALF_FLOAT:
          return t.webgl2
            ? R.WebGLConstants.HALF_FLOAT
            : R.WebGLConstants.HALF_FLOAT_OES
  ...
  class K {
    ...
    _scan(_, R = 0) {
      ...
      let e = 0
      for (; this._dataView.getUint8(this._offset) !== R && e < _; )
        e++, this._offset++
      return (
        e < _ && this._offset++,
        new Uint8Array(this._dataView.buffer, this._dataView.byteOffset + t, e)
      )
    }
  }
  ...
  function V(R, e) {
    ...
    try {
      V = (function (_) {
        ...
        for (let _ = 0; _ < S; _++) {
          ...
          R.channelType & G
            ? ((R.sampleLower = o._nextInt32()),
              (R.sampleUpper = o._nextInt32()))
            : ((R.sampleLower = o._nextUint32()),
              (R.sampleUpper = o._nextUint32())),
            (a.samples[_] = R)
        }
        ...
        for (; U._offset < r; ) {
          ...
          ;(t.keyValue[e] = e.match(/^ktx/i) ? s(A) : A),
            U._offset % 4 && U._skip(4 - (U._offset % 4))
        }
        if (N <= 0) return t
    ...
    if (0 !== V.layerCount)
      throw new t.RuntimeError('KTX2 texture arrays are not supported.')
    if (0 !== V.pixelDepth)
      throw new t.RuntimeError('KTX2 3D textures are unsupported.')
    ...
    return (
      0 !== V.vkFormat || (o.colorModel !== F && o.colorModel !== N)
        ...
        : (function (R, e, A, T, O, B) {
            ...
            if (!(M > 0 && K > 0 && C > 0))
              throw (
                (G.close(), G.delete(), new t.RuntimeError('Invalid KTX2 file'))
              )
            let i, V
            ...
            if (o.colorModel === F)
              if (A.etc)
                (i = s ? n.RGBA8_ETC2_EAC : n.RGB8_ETC2),
                  (V = s ? a.cTFETC2_RGBA : a.cTFETC1_RGB)
              else if (A.etc1 && !s) (i = n.RGB_ETC1), (V = a.cTFETC1_RGB)
              else if (A.s3tc)
                (i = s ? n.RGBA_DXT5 : n.RGB_DXT1),
                  (V = s ? a.cTFBC3_RGBA : a.cTFBC1_RGB)
              else if (A.pvrtc)
                (i = s ? n.RGBA_PVRTC_4BPPV1 : n.RGB_PVRTC_4BPPV1),
                  (V = s ? a.cTFPVRTC1_4_RGBA : a.cTFPVRTC1_4_RGB)
              else if (A.astc) (i = n.RGBA_ASTC), (V = a.cTFASTC_4x4_RGBA)
              else {
                if (!A.bc7)
                  throw new t.RuntimeError(
            ...
            if (!G.startTranscoding())
              throw (
                (G.close(),
                G.delete(),
                new t.RuntimeError('startTranscoding() failed'))
              )
            for (let R = 0; R < e.levels.length; ++R) {
              ...
              if (!_.defined(C))
                throw new t.RuntimeError('transcodeImage() failed.')
              O.push(n.buffer),
                (A[r[0]] = {
                  internalFormat: i,
                  width: M,
