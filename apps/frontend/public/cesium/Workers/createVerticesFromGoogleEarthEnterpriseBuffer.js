define([
  ...
  function N(t, e, n) {
    ...
    const i = t.length
    for (let o = 0; o < i; ++o)
      if (n.equalsEpsilon(t[o], e, a.CesiumMath.EPSILON12)) return o
    return -1
  }
  ...
  function A(t, e, r, s, c, u, h, l, d, g, m) {
    ...
    for (let I = 0; I < p; ++I) {
      ...
      const B = h.minimum,
        P = h.maximum
      i.Cartesian3.minimumByComponent(b, B, B),
        i.Cartesian3.maximumByComponent(b, P, P)
  ...
  return h(function (h, l) {
    ;(h.ellipsoid = i.Ellipsoid.clone(h.ellipsoid)),
      (h.rectangle = n.Rectangle.clone(h.rectangle))
    const d = (function (h, l, d, g, m, p, I, E, y, R, _) {
        ...
        y &&
          ...
          (D =
            1 /
            (u.WebMercatorProjection.geodeticLatitudeToMercatorAngle(O) - j)))
        const G = 1 !== p,
          z = new DataView(h)
        let q = Number.POSITIVE_INFINITY,
          J = Number.NEGATIVE_INFINITY
        const K = w
        ;(K.x = Number.POSITIVE_INFINITY),
          (K.y = Number.POSITIVE_INFINITY),
          ...
          for (let t = 0; t < g; ++t) {
            ...
            ;(p[t] = mt),
              Math.abs(c - W) < s
                ...
                : Math.abs(g - O) < l &&
                  gt.push({ index: mt, cartographic: i.Cartographic.clone(S) }),
              (q = Math.min(m, q)),
              (J = Math.max(m, J)),
              (at[mt] = m)
            ...
            if (
              ...
              y &&
                (st[mt] =
                  (u.WebMercatorProjection.geodeticLatitudeToMercatorAngle(g) -
                    j) *
                  D),
              G)
            ) {
            ...
            n.Matrix4.multiplyByPoint(L, I, b),
              i.Cartesian3.minimumByComponent(b, K, K),
              i.Cartesian3.maximumByComponent(b, Q, Q)
            let E = (c - W) / (F - W)
            E = a.CesiumMath.clamp(E, 0, 1)
            let T = (g - v) / (O - v)
            ;(T = a.CesiumMath.clamp(T, 0, 1)),
              (rt[mt] = new n.Cartesian2(E, T)),
              ++mt
          }
          const I = 3 * m
          for (let t = 0; t < I; ++t, ++pt)
            (ut[pt] = p[z.getUint16($, !0)]), ($ += T)
        ...
        const xt = new r.EllipsoidalOccluder(
            d
          ).computeHorizonCullingPointPossiblyUnderEllipsoid(l, ot, q),
          Nt = new t.AxisAlignedBoundingBox(K, Q, l),
          St = new r.TerrainEncoding(l, Nt, Tt.hMin, J, H, !1, y, G, p, I),
          bt = new Float32Array(tt * St.stride)
        let wt = 0
        for (let t = 0; t < tt; ++t)
          wt = St.encode(bt, wt, ot[t], rt[t], at[t], void 0, st[t], ct[t])
        const Bt = ht
            .map(function (t) {
              return t.index
            })
