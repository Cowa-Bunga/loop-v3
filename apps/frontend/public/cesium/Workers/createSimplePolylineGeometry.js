define([
  ...
  function P(l) {
    ...
    ;(s += e.defined(i) ? 1 + i.length * r.Color.packedLength : 1),
      (this.packedLength = s + o.Ellipsoid.packedLength + 3)
  }
  ;(P.pack = function (t, l, n) {
    ...
    const p = t._colors
    for (
      s = e.defined(p) ? p.length : 0, l[n++] = s, i = 0;
      i < s;
      ++i, n += r.Color.packedLength
    )
      r.Color.pack(p[i], l, n)
    return (
    ...
    (P.createGeometry = function (y) {
      ...
      if (h === t.ArcType.GEODESIC || h === t.ArcType.RHUMB) {
        ...
        h === t.ArcType.GEODESIC
          ? ((o = p.CesiumMath.chordLength(C, g.maximumRadius)),
            (l = c.PolylinePipeline.numberOfPoints),
            (n = c.PolylinePipeline.generateArc))
          : ((o = C),
            (l = c.PolylinePipeline.numberOfPointsRhumbLine),
            (n = c.PolylinePipeline.generateRhumbArc))
        const i = c.PolylinePipeline.extractHeights(d, g),
          a = B
        if (
          (h === t.ArcType.GEODESIC ? (a.minDistance = T) : (a.granularity = C),
          (a.ellipsoid = g),
          P)
        ) {
          let t = 0
        ...
        ;(D = P ? 2 * b - 2 : b),
          (k = new Float64Array(3 * D)),
          (G = e.defined(u) ? new Uint8Array(4 * D) : void 0)
        let t = 0,
          l = 0
        for (E = 0; E < b; ++E) {
          const n = d[E]
      ...
      ;(V.position = new i.GeometryAttribute({
