define([
  ...
  function x(e, t, o, y, u) {
    const d = n.EllipsoidTangentPlane.fromPoints(t, e).projectPointsOntoPlane(
      t,
      H
    )
    let f, h
    g.PolygonPipeline.computeWindingOrder2D(d) === g.WindingOrder.CLOCKWISE &&
      (d.reverse(), (t = t.slice().reverse()))
    let m = t.length,
      b = 0
    if (y)
  ...
  return (
    ...
    (O.createGeometry = function (t) {
      ...
      if (
        ...
        !d.CesiumMath.equalsEpsilon(P, A, 0, d.CesiumMath.EPSILON2)
      )
        ...
        for (_ = 0; _ < f.length; _++) {
          if (
            ((h = x(i, f[_], b, l, p)),
            (h.geometry.attributes.position.values =
