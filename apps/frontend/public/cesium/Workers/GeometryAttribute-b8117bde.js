define([
  ...
  const N = new n.Cartographic(),
    T = new n.Cartesian3(),
    l = new e.Matrix4(),
    c = [new n.Cartographic(), new n.Cartographic(), new n.Cartographic()],
    L = [new e.Cartesian2(), new e.Cartesian2(), new e.Cartesian2()],
    E = [new e.Cartesian2(), new e.Cartesian2(), new e.Cartesian2()],
    f = new n.Cartesian3(),
    p = new i.Quaternion(),
    m = new e.Matrix4(),
    y = new e.Matrix2()
  ;(I._textureCoordinateRotationPoints = function (t, a, r, o) {
    ...
    const u = e.Rectangle.center(o, N),
      I = n.Cartographic.toCartesian(u, r, T),
      C = i.Transforms.eastNorthUpToFixedFrame(I, r, l),
      b = e.Matrix4.inverse(C, l),
      h = L,
      A = c
    ;(A[0].longitude = o.west),
      (A[0].latitude = o.south),
      (A[1].longitude = o.west),
      (A[1].latitude = o.north),
      (A[2].longitude = o.east),
      (A[2].latitude = o.south)
    let x = f
    for (s = 0; s < 3; s++)
      n.Cartographic.toCartesian(A[s], r, x),
        (x = e.Matrix4.multiplyByPointAsVector(b, x, x)),
        (h[s].x = x.x),
        (h[s].y = x.y)
    const S = i.Quaternion.fromAxisAngle(n.Cartesian3.UNIT_Z, -a, p),
      d = n.Matrix3.fromQuaternion(S, m),
      P = t.length
    ...
    for (s = 0; s < P; s++)
      (x = e.Matrix4.multiplyByPointAsVector(b, t[s], x)),
        (x = n.Matrix3.multiplyByVector(d, x, x)),
        (G = Math.min(G, x.x)),
        (R = Math.min(R, x.y)),
        (_ = Math.max(_, x.x)),
        (O = Math.max(O, x.y))
    ...
    for (s = 0; s < 3; s++) {
      const t = w[s]
      e.Matrix2.multiplyByVector(g, t, t),
        (t.x = (t.x - V.x) / M),
        (t.y = (t.y - V.y) / v)
    }
    const F = w[0],
      ...
      B = new Array(6)
    return (
      e.Cartesian2.pack(F, B),
      e.Cartesian2.pack(W, B, 2),
      e.Cartesian2.pack(Y, B, 4),
      B
    )
  }),
    (t.Geometry = I),
