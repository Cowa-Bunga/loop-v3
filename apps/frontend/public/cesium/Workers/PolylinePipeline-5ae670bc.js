define([
  ...
  const y = new t.Cartographic(),
    ...
    M = new t.Cartesian3(),
    R = new t.Cartesian3(),
    S = new n.EllipsoidGeodesic()
  let b = new i.EllipsoidRhumbLine()
  function x(e, a, n, i, r, o, s, c) {
    const u = i.scaleToGeodeticSurface(e, M),
      h = i.scaleToGeodeticSurface(a, R),
      f = l.numberOfPoints(e, a, n),
      g = i.cartesianToCartographic(u, y),
      C = i.cartesianToCartographic(h, A),
      p = T(f, r, o)
    S.setEndPoints(g, C)
    const d = S.surfaceDistance / f
    let m = c
    g.height = r
    let w = i.cartographicToCartesian(g, E)
    t.Cartesian3.pack(w, s, m), (m += 3)
    for (let e = 1; e < f; e++) {
      const a = S.interpolateUsingSurfaceDistance(e * d, A)
      ;(a.height = p[e]),
  ...
  function D(e, a, n, r, o, s, c, u) {
    const h = r.cartesianToCartographic(e, y),
      f = r.cartesianToCartographic(a, A),
      g = l.numberOfPointsRhumbLine(h, f, n)
    ;(h.height = 0), (f.height = 0)
    const C = T(g, o, s)
    b.ellipsoid.equals(r) || (b = new i.EllipsoidRhumbLine(void 0, void 0, r)),
      b.setEndPoints(h, f)
    const p = b.surfaceDistance / g
    let d = u
    h.height = o
    let m = r.cartographicToCartesian(h, E)
    t.Cartesian3.pack(m, c, d), (d += 3)
    for (let e = 1; e < g; e++) {
      const a = b.interpolateUsingSurfaceDistance(e * p, A)
      ;(a.height = C[e]),
        (m = r.cartographicToCartesian(a, E)),
  ...
  ;(l.wrapLongitude = function (e, n) {
    ...
    if (a.defined(e) && e.length > 0) {
      n = a.defaultValue(n, s.Matrix4.IDENTITY)
      const l = s.Matrix4.inverseTransformation(n, h),
        u = s.Matrix4.multiplyByPoint(l, t.Cartesian3.ZERO, f),
        P = t.Cartesian3.normalize(
          s.Matrix4.multiplyByPointAsVector(l, t.Cartesian3.UNIT_Y, g),
          g
        ),
        T = c.Plane.fromPointNormal(u, P, C),
        y = t.Cartesian3.normalize(
          s.Matrix4.multiplyByPointAsVector(l, t.Cartesian3.UNIT_X, p),
          p
        ),
        A = c.Plane.fromPointNormal(u, y, d)
      let E = 1
      ...
      for (let n = 1; n < R; ++n) {
        const s = e[n]
        if (
          c.Plane.getPointDistance(A, M) < 0 ||
          c.Plane.getPointDistance(A, s) < 0
        ) {
          const e = r.IntersectionTests.lineSegmentPlane(M, s, T, m)
          if (a.defined(e)) {
            const a = t.Cartesian3.multiplyByScalar(P, 5e-9, w)
            c.Plane.getPointDistance(T, M) < 0 && t.Cartesian3.negate(a, a),
        ...
        i.push(t.Cartesian3.clone(e[n])), E++, (M = s)
      }
      o.push(E)
    }
    ...
    (l.generateArc = function (e) {
      ...
      let s = a.defaultValue(e.height, 0)
      const c = Array.isArray(s)
      if (i < 1) return []
      if (1 === i) {
        const e = r.scaleToGeodeticSurface(n[0], M)
        if (((s = c ? s[0] : s), 0 !== s)) {
          const a = r.geodeticSurfaceNormal(e, E)
          t.Cartesian3.multiplyByScalar(a, s, a), t.Cartesian3.add(e, a, e)
        }
        return [e.x, e.y, e.z]
      }
      let u = e.minDistance
      if (!a.defined(u)) {
        const t = a.defaultValue(e.granularity, o.CesiumMath.RADIANS_PER_DEGREE)
        u = o.CesiumMath.chordLength(t, r.maximumRadius)
      }
      let h,
        f = 0
      ...
      let p = 0
      for (h = 0; h < i - 1; h++) {
        p = x(n[h], n[h + 1], u, r, c ? s[h] : s, c ? s[h + 1] : s, C, p)
      }
      P.length = 0
      const d = n[i - 1],
  ...
  ;(l.generateRhumbArc = function (e) {
    ...
    const u = a.defaultValue(e.granularity, o.CesiumMath.RADIANS_PER_DEGREE)
    let h,
      f,
      g = 0,
    ...
    let m = 0
    for (h = 0; h < i - 1; h++) {
      m = D(n[h], n[h + 1], u, r, c ? s[h] : s, c ? s[h + 1] : s, d, m)
    }
    P.length = 0
    const w = n[i - 1],
