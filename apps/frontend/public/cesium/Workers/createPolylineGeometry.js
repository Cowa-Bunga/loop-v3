define([
  ...
  function P(e, t, o, r, n) {
    ...
    const f = (u - s) / n,
      ...
      _ = (h - d) / n
    for (l = 0; l < n; l++)
      i[l] = new a.Color(s + l * f, c + l * C, p + l * g, d + l * _)
    return i
  }
  function x(r) {
  ...
  x.pack = function (o, r, n) {
    ...
    let s = l.length
    for (r[n++] = s, i = 0; i < s; ++i, n += t.Cartesian3.packedLength)
      t.Cartesian3.pack(l[i], r, n)
    const c = o._colors
    for (
      s = e.defined(c) ? c.length : 0, r[n++] = s, i = 0;
      i < s;
      ++i, n += a.Color.packedLength
    )
      a.Color.pack(c[i], r, n)
    return (
  ...
  x.unpack = function (o, r, n) {
    ...
    let l = o[r++]
    const s = new Array(l)
    for (i = 0; i < l; ++i, r += t.Cartesian3.packedLength)
      s[i] = t.Cartesian3.unpack(o, r)
    l = o[r++]
    const c = l > 0 ? new Array(l) : void 0
    for (i = 0; i < l; ++i, r += a.Color.packedLength)
      c[i] = a.Color.unpack(o, r)
    ...
    (x.createGeometry = function (u) {
      ...
      if (C === o.ArcType.GEODESIC || C === o.ArcType.RHUMB) {
        ...
        C === o.ArcType.GEODESIC
          ? ((t = p.CesiumMath.chordLength(g, _.maximumRadius)),
            (r = d.PolylinePipeline.numberOfPoints))
      ...
      for (x = 0; x < L; ++x) {
        ...
        0 === x
          ? ((M = D),
            t.Cartesian3.subtract(b[0], b[1], M),
            t.Cartesian3.add(b[0], M, M))
          : (M = b[x - 1]),
          t.Cartesian3.clone(M, V),
          t.Cartesian3.clone(b[x], k),
          x === L - 1
            ? ((M = D),
              t.Cartesian3.subtract(b[L - 1], b[L - 2], M),
              t.Cartesian3.add(b[L - 1], M, M))
            : (M = b[x + 1]),
          t.Cartesian3.clone(M, v),
          e.defined(B) &&
            ((o = 0 === x || f ? h[x] : h[x - 1]), x !== L - 1 && (r = h[x]))
        const n = x === L - 1 ? 2 : 4
        for (w = 0 === x ? 2 : 0; w < n; ++w) {
          t.Cartesian3.pack(k, G, U),
            t.Cartesian3.pack(V, O, U),
            t.Cartesian3.pack(v, R, U),
            (U += 3)
          const n = w - 2 < 0 ? -1 : 1
          if (
            ((I[N++] = (w % 2) * 2 - 1),
            (I[N++] = n * y),
            m.st && ((S[H++] = x / (L - 1)), (S[H++] = Math.max(I[N - 2], 0))),
            e.defined(B))
          ) {
            const e = w < 2 ? o : r
      ...
      ;(Y.position = new l.GeometryAttribute({
        componentDatatype: i.ComponentDatatype.DOUBLE,
        componentsPerAttribute: 3,
        values: G
      })),
        (Y.prevPosition = new l.GeometryAttribute({
          componentDatatype: i.ComponentDatatype.DOUBLE,
          componentsPerAttribute: 3,
          values: O
        })),
        (Y.nextPosition = new l.GeometryAttribute({
          componentDatatype: i.ComponentDatatype.DOUBLE,
          componentsPerAttribute: 3,
          values: R
        })),
        (Y.expandAndWidth = new l.GeometryAttribute({
          componentDatatype: i.ComponentDatatype.FLOAT,
          componentsPerAttribute: 2,
          values: I
        })),
        m.st &&
          (Y.st = new l.GeometryAttribute({
            componentDatatype: i.ComponentDatatype.FLOAT,
            componentsPerAttribute: 2,
            values: S
          })),
