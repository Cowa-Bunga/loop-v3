define([
  ...
  ;(m.getRegularGridAndSkirtIndicesAndEdgeIndices = function (e, t) {
    ...
    (m.getEstimatedLevelZeroGeometricErrorForAHeightmap = function (e, t, r) {
      return (
        (2 * e.maximumRadius * Math.PI * m.heightmapTerrainQuality) / (t * r)
      )
    }),
    (m.prototype.requestTileGeometry =
      s.DeveloperError.throwInstantiationError),
    (m.prototype.getLevelMaximumGeometricError =
      s.DeveloperError.throwInstantiationError),
    (m.prototype.getTileDataAvailable =
      s.DeveloperError.throwInstantiationError),
    (m.prototype.loadTileDataAvailability =
      s.DeveloperError.throwInstantiationError)
  const w = 32767,
    M = new r.Cartesian3(),
    S = new r.Cartesian3(),
  ...
  function b(e, n, o, i, s, c, d, h, u) {
    ...
    for (let s = 0; s < E; ++s) {
      const E = e[s],
        p = o[E],
        f = i[E]
      ;(A.longitude = a.CesiumMath.lerp(T, m, f.x)),
        (A.latitude = a.CesiumMath.lerp(g, I, f.y)),
        (A.height = p - n)
  ...
  function W(e, t, r, o, i, s, d, h, u, l, I, g, m, T) {
    ...
    for (let n = 0; n < w; ++n) {
      const u = r[n],
        w = i[u],
        S = s[u]
      ;(A.longitude = a.CesiumMath.lerp(N, y, S.x) + m),
        (A.latitude = a.CesiumMath.lerp(f, p, S.y) + T),
        (A.height = w - l)
      ...
      o.hasWebMercatorT &&
        (b =
          (c.WebMercatorProjection.geodeticLatitudeToMercatorAngle(A.latitude) -
            I) *
          g),
  ...
  function v(e, t) {
    ...
    return (
      'function' == typeof e.slice &&
        ((r = e.slice()), 'function' != typeof r.sort && (r = void 0)),
  ...
  return d(function (d, h) {
    const u = d.quantizedVertices,
      ...
      g =
        d.westIndices.length +
        d.eastIndices.length +
        d.southIndices.length +
        d.northIndices.length,
    ...
    let O, B
    T &&
      ((O = c.WebMercatorProjection.geodeticLatitudeToMercatorAngle(P)),
      (B =
        1 / (c.WebMercatorProjection.geodeticLatitudeToMercatorAngle(k) - O)))
    ...
    let se
    if (H < 0) {
      se = new o.EllipsoidalOccluder(
        F
      ).computeHorizonCullingPointPossiblyUnderEllipsoid(G, Q, H)
    }
    let ce = H
    ;(ce = Math.min(
      ce,
      b(d.westIndices, d.westSkirtHeight, q, z, y, F, Y, Z, J)
    )),
      (ce = Math.min(
        ce,
        b(d.southIndices, d.southSkirtHeight, q, z, y, F, Y, Z, J)
      )),
      (ce = Math.min(
        ce,
        b(d.eastIndices, d.eastSkirtHeight, q, z, y, F, Y, Z, J)
      )),
      (ce = Math.min(
        ce,
        b(d.northIndices, d.northSkirtHeight, q, z, y, F, Y, Z, J)
      ))
