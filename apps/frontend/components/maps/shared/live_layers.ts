...
export const layers = ({ theme }) => {
  return [
    new IconLayer({
      id: 'map-markers',
      data: MOCK.map((v) => ({ ...v, label: v.timestamp })),
      pickable: true,
      ...
      getSize: (d) => 10,
      getColor: (d) => [255, (base = (base > 250 ? 250 : base) + 1), 255],
      opacity: 0.1
    }),
    ...
    new ColumnLayer({
      id: 'point-highlight',
      data: [
        { point: data[0].path[0], label: 'Start', color: [255, 0, 0] },
        { point: data[0].path[data[0].path.length - 1], label: 'Driver', color: [100, 200, 100] }
      ],
      radius: 40,
      opacity: 0.04,
